import { CleanCard, BrutalCard, PastelCard, NightCard } from "../components/ProjectCards";
                ...
                <div className="flex flex-row gap-4 overflow-hidden w-max p-4">
                    {
                        Object.keys(projects).map((key, i) => (
                            <CleanCard key={key} id={projects[i].id} title={projects[i].title || ""} summary={projects[i].summary || ""} date={convertTimestampToDate(projects[i].datetime)} link={projects[i].link} />
                        ))
                    }
                </div>
            </div>
